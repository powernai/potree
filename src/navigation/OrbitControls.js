...
export class OrbitControls extends EventDispatcher {
      ...
      if (
        ...
        if (cpmsRaycaster) {
          ...
          if (
            ...
            !raycast.object ||
            !(
              this.scene.pointclouds.includes(raycast.object) ||
              this.scene.pointclouds.includes(raycast.object.parent)
            )
          )
            return;
        }
  ...
  zoomToBIM(boundingBox) {
    ...
    const newCameraPosition = new THREE.Vector3().addVectors(
      boxCenter,
      direction.multiplyScalar(zoomedDistance)
    );

    // Animate the view position and target
    let value = { x: 0 };
    let startPos = view.position.clone();
    let targetPos = newCameraPosition.clone();
    let targetSideOffset = 0;
    let targetRadius = zoomedDistance;
    ...
    const tween = new TWEEN.Tween(value)
      ...
      .onUpdate(() => {
        ...
        view.sideOffset = (1 - t) * view.sideOffset + t * targetSideOffset;
        view.radius = (1 - t) * view.radius + t * targetRadius;
        view.upOffset = (1 - t) * view.upOffset + t * targetUpOffset;
        this.viewer.setMoveSpeed(view.radius);
      })
      .onComplete(() => {
        this.tweens = this.tweens.filter((e) => e !== tween);
        const sphere = this.scene.scene.children.find(
          (x) => x.isPivotIndicatorSphere
        );
        if (sphere) sphere.position.copy(this.scene.view.getPivot());
      })
      .start();
  }
